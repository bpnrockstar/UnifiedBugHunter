...
**At a glance:** `unified-bug-hunter` v4.3.2 · MIT · author `bpnrockstar` · **89 skills** · **35 commands** · **28 agents** · **~24 engine tools** · **6 MCP servers** · **372 tests** · Flask + SQLite dashboard

---

## What it is

UnifiedBugHunter is a Claude Code *plugin* that orchestrates real offensive tooling with judgment and safety — it does **not** replace the underlying tools. Instead of running `subfinder`, `httpx`, `nuclei`, `dalfox`, `sqlmap`, `TREVORspray`, `Foundry`, etc. by hand, it wraps them in a deterministic scope checker, an append-only audit trail, cross-target pattern learning, a Flask/SQLite dashboard, and a hardened linter + CI suite. It ships **89 methodology skills, 35 slash commands, 28 subagents, ~24 deterministic engine tools, and 6 MCP servers** — all governed so the operator gets cumulatively better and stays inside the rules.
...
| 2 | **Interface** | `commands/` (35 `.md`) | Slash-command entry points. Thin routers — invoke an engine script or dispatch to an agent/skill. Prefixed to avoid collisions (`/pickup` not `/resume`). |
| 3 | **Actor** | `agents/` (28 `.md`) | Stateful subagents that run multi-step loops, pick tools, and carry safety rails in their prompts. Pinned models (Haiku/Sonnet/Opus). |
...
graph TD
    ...
    subgraph L2[L2 Interface: commands/]
        CMD[35 slash commands]
    end
    subgraph L3[L3 Actor: agents/]
        AG[28 agents<br/>9 pipeline + 19 offensive]
        AUTO[autopilot loop]
    end
...
### Commands — 35 (8 model-driven, no backing script)

- **Recon:** `/recon`, `/cloud-recon`, `/osint-employees`, `/arsenal`.
- **Scope:** `/scope`, `/scope-aggregate`, `/surface`.
- **Hunting:** `/hunt`, `/autopilot`, `/dast-scan`, `/scan-cves`, `/code-audit`, `/graphql-audit`, `/secrets-hunt`, `/chain`.
- **Validation / Reporting:** `/triage`, `/validate`, `/report`, `/patch` (all model-driven).
...
### Agents — 28 (9 pipeline + 19 offensive)
...
- **Bug-bounty pipeline (9):** `recon-agent`, `recon-ranker`, `chain-builder`, `validator`, `report-writer`, `autopilot`, `web3-auditor`, `token-auditor`, `credential-hunter`.
...
### Engine tools — ~24 (in `tools/`)

Python on `python3`, shell on Bash. Orchestrator `hunt.py`; recon `recon_engine.sh`; scanning `vuln_scanner.sh`, `cve_scan.sh`, `dast_scanner.sh`, `graphql_audit.sh`; validation `validate.py`; intel `intel_engine.py`, `learn.py`; scope `scope_checker.py`, `scope_aggregator.sh`; discovery/bypass `secrets_hunter.sh`, `takeover_scanner.sh`, `cloud_recon.sh`, `param_discovery.sh`, `bypass_403.sh`; CI/CD `cicd_scanner.sh`; web3 `token_scanner.py`; LLM `llm_redteam.py` (+ `llm_payloads/` library, 7 JSON files); registry `external_arsenal.sh`; credential pipeline `wordlist_engine.sh`, `osint_employees.sh`, `breach_checker.py`, `spray_orchestrator.sh`. Credential gating: `wordlist_engine.sh`, `osint_employees.sh`, and `spray_orchestrator.sh` (TREVOR o365/okta modes) require `--with-credential-attack`.
...
- **Job 2 — `tests`** (Python 3.12) runs `pytest tests/ -q` — **23 pytest files / 372 test functions** — plus `bash tests/test_cicd_scanner.sh`. An `eval/` harness (PortSwigger labs, false-positive cases) measures real hunting accuracy out-of-band.
...
| Slash commands | **35** (8 model-driven, no backing script) |
| Agents | **28** = 9 bug-bounty pipeline + 19 offensive security |
| Engine tools | **~24** in `tools/` (+ `llm_payloads/` library, 7 JSON files) |
