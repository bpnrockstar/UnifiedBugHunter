...
#
# Unified-Bug-Hunter ships a frozen snapshot of YOUR_USER's skills and
...
# ~/.claude/. It clones UnifiedBugHunter into
...
# === UnifiedBugHunter (foundation) ===
if [ ! -d "$COMMUNITY_DIR/unified-bug-hunter" ]; then
  echo "Cloning UnifiedBugHunter..."
  git clone --depth=1 https://github.com/UnifiedBugHunter.git \
    "$COMMUNITY_DIR/unified-bug-hunter"
else
  echo "UnifiedBugHunter already cloned — pulling latest"
  ( cd "$COMMUNITY_DIR/unified-bug-hunter" && git pull --ff-only ) || true
fi
...
if [ -d "$HOME/.claude/skills/bug-bounty" ] && [ ! -L "$HOME/.claude/skills/bug-bounty" ]; then
  # Check if the existing one is YOUR_USER's or a custom local toolkit
  if grep -q "Master workflow" "$HOME/.claude/skills/bug-bounty/SKILL.md" 2>/dev/null; then
    echo "✓ YOUR_USER bug-bounty already installed"
  else
...
# === Run YOUR_USER's installer (which copies skills + commands) ===
echo "Running YOUR_USER installer..."
cd "$COMMUNITY_DIR/unified-bug-hunter"
chmod +x install.sh

# Pipe "n" to skip the optional interactive Burp MCP setup prompt
# (we'll handle Burp MCP setup separately in INSTALL.md)
echo "n" | ./install.sh || {
  echo "⚠ YOUR_USER installer reported errors — check output above"
...
echo "  3. (Optional) Install per-class hunt-* skills via YOUR_USER/public-skills-builder"
echo ""
