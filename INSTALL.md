...
Step-by-step setup for the Unified-Bug-Hunter skill bundle.
...
The bundle ships a frozen snapshot of YOUR_USER's skills. To pull the latest from upstream and re-bundle:
...
This clones `UnifiedBugHunter` into `~/security-research/community-skills/` and runs its installer. Useful when you want fresher hunt patterns; not needed for first-time setup.
...
```bash
cd ~/security-research
git clone https://github.com/YOUR_USER/public-skills-builder.git
