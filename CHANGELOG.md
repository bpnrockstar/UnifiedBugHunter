...
- **`tools/probe_scanner.py` / `tools/idor_vendor_test.py` help-path crashes** — both tools now handle missing `requests` gracefully and print useful `--help` output without importing the dependency first.
...
- Vulnerability scanners: `idor_scanner.py`, `mutation_idor.py`, `oauth_tester.py`, `race_tester.py`
- AI/LLM testing: `probe_scanner.py`, `payload_builder.py`, `browser_recon.js`
- Shell tools: `recon_engine.sh`, `vuln_scanner.sh`
- Utilities: `sneaky_bits.py`, `target_selector.py`, `zero_day_fuzzer.py`, `cve_scan.sh`
