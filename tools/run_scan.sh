#!/bin/bash
# Bug Bounty 20-Day Hunt — Master Runner
...
echo ""
CMD="python3 $TOOLS_DIR/idor_scanner.py \
  --token-a $TOKEN_A \
  --token-b $TOKEN_B"
...
echo ""
python3 "$TOOLS_DIR/oauth_tester.py" \
  --check-cors \
  --check-oauth \
  --check-ssrf \
  2>&1 | tee -a "$LOG"
...
if [[ -n "$REPORT_ID" ]]; then
  python3 "$TOOLS_DIR/race_tester.py" \
    --token-a "$TOKEN_A" \
...
echo "  [ ] 2FA rate limit — run: python3 race_tester.py --token-a TOKEN_B --test 2fa"
