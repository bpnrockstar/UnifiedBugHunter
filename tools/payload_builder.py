#!/usr/bin/env python3
"""
payload_builder.py — VAPT Payload Library + LLM Injection Generator
...
Usage — VAPT payloads:
  python3 payload_builder.py --list                         List all payload categories
  python3 payload_builder.py --type nosql                   Print NoSQL injection payloads
  python3 payload_builder.py --type ssti                    Print SSTI detection + RCE payloads
  python3 payload_builder.py --type cmd                     Command injection + bypass techniques
  python3 payload_builder.py --type mfa                     MFA/2FA bypass payloads
  python3 payload_builder.py --type saml                    SAML/SSO attack payloads
  python3 payload_builder.py --type smuggling               HTTP request smuggling templates
  python3 payload_builder.py --type websocket               WebSocket injection payloads
  python3 payload_builder.py --type all --output-dir out/   Export all categories to files

Usage — LLM injection:
  python3 payload_builder.py --attack system_prompt
  python3 payload_builder.py --attack all --output-dir payloads/
"""
...
# Source: UnifiedBugHunter v2.1.0 + OBSIDIAN extensions
# ════════════════════════════════════════════════════════════════════════════════
