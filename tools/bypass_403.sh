...
# Confidence tiers for bypass verdicts
CONFIRMED='[CONFIRMED]'
POSSIBLE='[POSSIBLE]'
...
# ---------------------------------------------------------------------------
# Response Body Normalisation — strips dynamic content (timestamps, tokens,
# nonces, CSRF strings) so baseline-vs-bypass comparison avoids false
# positives from session-specific or time-sensitive page elements.
# ---------------------------------------------------------------------------
_normalize_body() {
  local body_file="$1"
  sed -e 's/[0-9]\{10,\}/TIMESTAMP/g' \
      -e 's/[a-f0-9]\{32,\}/HASH32/g' \
      -e 's/[a-f0-9]\{40,\}/HASH40/g' \
      -e 's/[a-f0-9]\{64,\}/HASH64/g' \
      -e 's/csrf[-_][a-zA-Z0-9._-]\{8,\}/CSRF_TOKEN/g' \
      -e 's/nonce[-_][a-zA-Z0-9._-]\{8,\}/NONCE/g' \
      -e 's/__VIEWSTATE[-_a-zA-Z0-9+/=]*\{40,\}/VIEWSTATE/g' \
      -e 's/auth_token[-_=][a-zA-Z0-9._-]\{8,\}/AUTH_TOKEN/g' \
...
_probe_one() {
  ...
  local orig_norm="" bypass_norm="" baseline_code=""
  ...
  for combo in \
    ...
    # Normalize for dynamic-content comparison
    bypass_norm=$(_normalize_body "$_tmpbody")
    [ -z "$orig_norm" ] && { orig_norm="$bypass_norm"; baseline_code=$orig_code; orig_code="$code"; }
    local _tier="$CONFIRMED"
    [ "$bypass_norm" = "$orig_norm" ] && [ "$code" = "$baseline_code" ] && _tier="$POSSIBLE"
    [ "$_verdict" = "blocked" ] && _tier="$INFORMATIONAL"
    case "$_verdict" in
      bypassed)
        local _reason
        _reason=$(printf '%s' "$_verdict_json" | grep -oE '"reason":[[:space:]]*"[^"]*"' | grep -oE '"[^"]*"$' | tr -d '"')
        hit "$_tier $method  $url  $hdr  → HTTP $code (${_body_len}B) [${_reason}]"
        [ -n "$_log_ids" ] && log "  WAF Log IDs: $_log_ids"
        echo "$_tier|$method|$url|$hdr|$code|$_body_len|$_verdict_json" >> "$OUT_DIR/bypass_hits.txt"
        found=1
        ;;
      needs_review)
        echo -e "${YELLOW}[?]${NC} $_tier $method  $url  $hdr  → HTTP $code (${_body_len}B) — uncertain"
        echo "$_tier|$method|$url|$hdr|$code|$_body_len|$_verdict_json" >> "$OUT_DIR/bypass_uncertain.txt"
        ;;
      blocked)
        echo "$_tier|$method|$url|$hdr|$code|$_body_len|$_verdict_json" >> "$OUT_DIR/bypass_blocked.txt"
        ;;
    esac
